...
public interface UniqueMorseCodeWords {
    ...
    class Solution implements UniqueMorseCodeWords {
        ...
        public int uniqueMorseRepresentations(String[] words) {
            ...
            Set<String> morseCodeSet = new HashSet<>(words.length);
            for (String word : words) {
                StringBuilder morse = new StringBuilder(word.length() * 4);
                for (int i = 0; i < word.length(); i++) {
                    morse.append(morseCode[word.charAt(i) - 'a']);
                }
                morseCodeSet.add(morse.toString());
            }
