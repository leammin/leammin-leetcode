...
public interface FindSubarraysWithEqualSum {
    ...
    static Testsuite<FindSubarraysWithEqualSum> testsuite() {
        return Testsuite.<FindSubarraysWithEqualSum>builder()
                .addExpected(t -> t.findSubarrays(new int[]{4,2,4}), true)
                .addExpected(t -> t.findSubarrays(new int[]{1,2,3,4,5}), false)
                .addExpected(t -> t.findSubarrays(new int[]{0,0,0}), true)
                .build();
    }

    class Solution implements FindSubarraysWithEqualSum {

        @Override
        public boolean findSubarrays(int[] nums) {
            Set<Integer> s = new HashSet<>();
            for (int i = 1; i < nums.length; i++) {
                if (!s.add(nums[i - 1] + nums[i])) {
                    return true;
                }
            }
            return false;
