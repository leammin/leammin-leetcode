...
/**
 * <a href="https://leetcode.cn/problems/divisible-and-non-divisible-sums-difference/">2894. 分类求和并作差</a>
 * 
...
public interface DivisibleAndNonDivisibleSumsDifference {
    ...
    static Testsuite<DivisibleAndNonDivisibleSumsDifference> testsuite() {
        return Testsuite.<DivisibleAndNonDivisibleSumsDifference>builder()
                .addExpected(t -> t.differenceOfSums(10, 3), 19)
                .addExpected(t -> t.differenceOfSums(5, 6), 15)
                .addExpected(t -> t.differenceOfSums(5, 1), -15)
                .build();
    }

    class Solution implements DivisibleAndNonDivisibleSumsDifference {

        @Override
        public int differenceOfSums(int n, int m) {
            int sumn = (1 + n) * n / 2;
            int countm = n / m;
            int summ = (1 + countm) * countm / 2* m;
            return sumn - 2*summ;
        }
    }
}
