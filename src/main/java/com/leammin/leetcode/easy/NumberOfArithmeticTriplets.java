...
public interface NumberOfArithmeticTriplets {
    ...
    static Testsuite<NumberOfArithmeticTriplets> testsuite() {
        return Testsuite.<NumberOfArithmeticTriplets>builder()
                .addExpected(t -> t.arithmeticTriplets(new int[]{0,1,4,6,7,10}, 3), 2)
                .addExpected(t -> t.arithmeticTriplets(new int[]{4,5,6,7,8,9}, 2), 2)
                .build();
    }

    class Solution implements NumberOfArithmeticTriplets {
        ...
        public int arithmeticTriplets(int[] nums, int diff) {
            ...
            for (int num : nums) {
                existed[num] = true;
                if (num >= 2 * diff && existed[num - diff] && existed[num - 2 * diff]) {
                    res++;
                }
            }
            return res;
