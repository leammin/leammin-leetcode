...
public interface MinimumCostForTickets {
    ...
    class Solution implements MinimumCostForTickets {
        ...
        public int mincostTickets(int[] days, int[] costs) {
            ...
            int res = mincostTickets(dp, days, costs, 0);
            return res;
        }

        private int mincostTickets(int[] dp, int[] days, int[] costs, int cur) {
            if (cur >= days.length) {
                return 0;
            }
            if (dp[cur] != 0) {
            ...
            int one = one(dp, days, costs, cur);
            int seven = seven(dp, days, costs, cur);
            int thirty = thirty(dp, days, costs, cur);

            return dp[cur] = Math.min(one, Math.min(seven, thirty));
        }

        private int one(int[] dp, int[] days, int[] costs, int cur) {
            return costs[0] + mincostTickets(dp, days, costs, cur + 1);
        }

        private int seven(int[] dp, int[] days, int[] costs, int cur) {
            int max = days[cur] + 7 - 1;
            int i = Arrays.binarySearch(days, max) + 1;
            return costs[1] + mincostTickets(dp, days, costs, Math.abs(i));
        }

        private int thirty(int[] dp, int[] days, int[] costs, int cur) {
            int max = days[cur] + 30 - 1;
            int i = Arrays.binarySearch(days, max) + 1;
