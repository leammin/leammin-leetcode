...
public interface MaskingPersonalInformation {
    ...
    static Testsuite<MaskingPersonalInformation> testsuite() {
        return Testsuite.<MaskingPersonalInformation>builder()
                .addExpected(t -> t.maskPII("LeetCode@LeetCode.com"), "l*****e@leetcode.com")
                .addExpected(t -> t.maskPII("AB@qq.com"), "a*****b@qq.com")
                .addExpected(t -> t.maskPII("1(234)567-890"), "***-***-7890")
                .addExpected(t -> t.maskPII("86-(10)12345678"), "+**-***-***-5678")
                .build();
    }

    class Solution implements MaskingPersonalInformation {
        ...
        public String maskPII(String s) {
            ...
            for (int i = cs.length - 1; i >= 0; i--) {
                char c = cs[i];
                if (c == '@') {
                    return (cs[0] + "*****" + cs[i - 1] + String.valueOf(cs, i, cs.length - i)).toLowerCase();
                }
                if (c >= '0' && c <= '9' && d++ < 4) {
                    x[4 - d] = c;
                }
...
class MaskingPersonalInformationTest extends AbstractTest<MaskingPersonalInformation> {
}
