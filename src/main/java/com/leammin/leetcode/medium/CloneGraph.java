...
public interface CloneGraph {
    ...
    class Solution implements CloneGraph {
        ...
        public Node cloneGraph(Node node) {
            return clone(new Node[101], node);
        }

        private Node clone(Node[] nodes, Node node) {
            if (node == null) {
                return null;
            }
            ...
            Node clone = nodes[node.val] = new Node(node.val);
            for (Node neighbor : node.neighbors) {
                clone.neighbors.add(clone(nodes, neighbor));
            }
            return clone;
        }
    }
