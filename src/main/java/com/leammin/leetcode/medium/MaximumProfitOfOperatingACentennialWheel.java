 ...
 * @date 2023-03-05
 */
public interface MaximumProfitOfOperatingACentennialWheel {
    int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost);

    static Testsuite<MaximumProfitOfOperatingACentennialWheel> testsuite() {
        return Testsuite.<MaximumProfitOfOperatingACentennialWheel>builder()
                .addExpected(t -> t.minOperationsMaxProfit(new int[]{8,3}, 5, 6), 3)
                .addExpected(t -> t.minOperationsMaxProfit(new int[]{10,9,6}, 6, 4), 7)
                .addExpected(t -> t.minOperationsMaxProfit(new int[]{3,4,0,5,1}, 1, 92), -1)
                .addExpected(t -> t.minOperationsMaxProfit(new int[]{10,10,6,4,7}, 3, 8), 9)
                .build();
    }

    class Solution implements MaximumProfitOfOperatingACentennialWheel {
        ...
        public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
            ...
            if (waiting > 0) {
                int run = waiting / 4;
                waiting %= 4;
                profit += 4 * run * boardingCost - run * runningCost;
                if (profit > maxProfit) {
                    maxProfit = profit;
                    res = customers.length + run;
                }
                profit += waiting * boardingCost - runningCost;
                if (profit > maxProfit) {
                    maxProfit = profit;
                    res = customers.length + run + 1;
                }
            }
            return res;
        }
...
class MaximumProfitOfOperatingACentennialWheelTest extends AbstractTest<MaximumProfitOfOperatingACentennialWheel> {
}
