...
public interface IsGraphBipartite {
    ...
    static Testsuite<IsGraphBipartite> testsuite() {
    ...
    class Solution implements IsGraphBipartite {

        @Override
        public boolean isBipartite(int[][] graph) {
            int[] marks = new int[graph.length];
            for (int i = 0; i < graph.length; i++) {
                if (marks[i] == 0 && !mark(graph, marks, i, 1)) {
                    return false;
                }
            }
            return true;
        ...
        private boolean mark(int[][] graph, int[] marks, int i, int m) {
            if (marks[i] == 0) {
                marks[i] = m;
                for (int j : graph[i]) {
                    if (!mark(graph, marks, j, -m)) {
                        return false;
                    }
                }
            }
