...
public interface ReformatPhoneNumber {
    ...
    class Solution implements ReformatPhoneNumber {
        ...
        public String reformatNumber(String number) {
            StringBuilder result = new StringBuilder(number.length() * 4 / 3 + 1);
            int n = 0;
            ...
            if (result.charAt(result.length() - 2) == '-') {
                result.setCharAt(result.length() - 2, result.charAt(result.length() - 3));
                result.setCharAt(result.length() - 3, '-');
            }
